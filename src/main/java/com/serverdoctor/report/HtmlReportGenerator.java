...
public class HtmlReportGenerator {
    ...
    public File generate(File outputDir,
        ...
        try {
            ...
            writer.write("<title>Server Doctor V0.2</title>");
            writer.write("<style>");
            ...
            writer.write(".muted{color:#666}.metric{font-size:24px;font-weight:bold;}");
            writer.write("</style></head><body>");

            writer.write("<h1>Server Doctor V0.2 诊断报告</h1>");
    ...
    private void writeJvm(FileWriter writer, JvmMetrics metrics) throws IOException {
        writer.write("<div class='card'><h2>JVM Heap / GC</h2>");

        if (metrics == null || !metrics.isAvailable()) {
            writer.write("<p class='WARN'>未能采集JVM指标：" +
        ...
        writer.write("<tr><td>Heap使用</td><td>" +
                format(toMb(metrics.getHeapUsedKb())) + " MB / " +
                format(toMb(metrics.getHeapCapacityKb())) + " MB (" +
                format(metrics.getHeapUsagePercent()) + "%)</td></tr>");
        writer.write("<tr><td>Metaspace使用</td><td>" +
                format(toMb(metrics.getMetaspaceUsedKb())) + " MB / " +
                format(toMb(metrics.getMetaspaceCapacityKb())) + " MB (" +
                format(metrics.getMetaspaceUsagePercent()) + "%)</td></tr>");
        writer.write("<tr><td>Young GC</td><td>" +
                metrics.getYoungGcCount() + " 次 / " +
                format(metrics.getYoungGcTimeSeconds()) + " 秒</td></tr>");
        writer.write("<tr><td>Full GC</td><td>" +
                metrics.getFullGcCount() + " 次 / " +
                format(metrics.getFullGcTimeSeconds()) + " 秒</td></tr>");
        writer.write("<tr><td>总GC时间</td><td>" +
                format(metrics.getTotalGcTimeSeconds()) + " 秒</td></tr>");
        writer.write("</table></div>");
    }
    ...
    private void writeThreadAnalysis(FileWriter writer,
        ...
        writer.write("<h3>持续RUNNABLE热点候选</h3>");
        if (analysis.getPersistentRunnableThreads().isEmpty()) {
            writer.write("<p>未发现满足当前规则的持续RUNNABLE热点候选。</p>");
        } else {
            ...
            writer.write("<p class='muted'>说明：该项是多次jstack得到的持续RUNNABLE候选，并不等同于真实CPU采样结果。</p>");
        }
        ...
        if (analysis.getStackGroups().isEmpty()) {
            ...
            writer.write("<table><tr><th>状态</th><th>线程数</th><th>顶部调用</th></tr>");
            for (ThreadAnalysis.StackGroup group : analysis.getStackGroups()) {
                writer.write("<tr><td>" + escape(group.getState()) + "</td><td>" +
